        ...
        for(int a = 0;a<podaci.size();a++)
        {
			...
			e.printStackTrace();
                ...
                try
                {
                    ...
                    SQLQuery q1=s.createSQLQuery("select * from korisnici where username = '"+podaci.get(2)+"' and korisnikID <>'"+podaci.get(4)+"'").addEntity("korisnici",Korisnici.class);

                    List<Korisnici> rows = q1.list();
                    if(rows.size()>0)
                    {
                        response.sendRedirect("ServletIzmenaProfila?zauzetUser=da&id="+podaci.get(4));
                        return;
                    }
                    ...
                    if(imgpa.length()==21)
                    {
                        ...
                        {
                            SQLQuery q=s.createSQLQuery("update korisnici set ime='"+podaci.get(0)+"', prezime='"+podaci.get(1)+"', username='"+podaci.get(2)+"'"
                                + " where korisnikID='"+podaci.get(4)+"'");
                            q.executeUpdate();
                            tr.commit();
                        }
                        else
                        {
                            SQLQuery q=s.createSQLQuery("update korisnici set ime='"+podaci.get(0)+"', prezime='"+podaci.get(1)+"', username='"+podaci.get(2)+"',password='"+sha1+"'"
                                + " where korisnikID='"+podaci.get(4)+"'");
                            q.executeUpdate();
                            tr.commit();
                        }
                    ...
                    else
                    {
                        if(podaci.get(3).equals(""))
                        {
                            SQLQuery q=s.createSQLQuery("update korisnici set imgPath='"+imgpa+"', ime='"+podaci.get(0)+"', prezime='"+podaci.get(1)+"', username='"+podaci.get(2)+"'"
                                + " where korisnikID='"+podaci.get(4)+"'");
                            q.executeUpdate();
                            tr.commit();
                        }
                        ...
                        {
                            SQLQuery q=s.createSQLQuery("update korisnici set imgPath='"+imgpa+"', ime='"+podaci.get(0)+"', prezime='"+podaci.get(1)+"', username='"+podaci.get(2)+"',password='"+sha1+"'"
                                + " where korisnikID='"+podaci.get(4)+"'");
                            q.executeUpdate();
                            tr.commit();
                        }
