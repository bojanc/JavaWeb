 ...
 * @author Bojan
 */
public class ServletAdminDodajKuciste extends HttpServlet {
        ...
        for(int a = 0;a<podaci.size();a++)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        SQLQuery q=s.createSQLQuery("insert into kuciste(naziv,dimenzije,imgPath)"
                            + "VALUES('"+podaci.get(0)+"', '"+podaci.get(1)+"', '"+imgpa+"')");
                        q.executeUpdate();
                        tr.commit();
                    }
                    
                    s.close();
                    response.sendRedirect("ServletAdminPrikazDelova?deo=kuciste");
                    return;
                }
