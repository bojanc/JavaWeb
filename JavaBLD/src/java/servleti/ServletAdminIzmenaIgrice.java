 ...
 * @author Bojan
 */
public class ServletAdminIzmenaIgrice extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(!(nazivF.equals("images")))
        {
            ...
            try
            {
                ...
                if(imgpa.length()==21)
                {
                    SQLQuery q=s.createSQLQuery("update igrice set igricaNaziv='"+podaci.get(0)+"', cpuPreID='"+podaci.get(1)+"', gpuPreID='"+podaci.get(2)+"',ramPreID='"+podaci.get(3)+"' where igricaID='"+podaci.get(4)+"'");
                    q.executeUpdate();
                    tr.commit();
                }
                else
                {
                    SQLQuery q=s.createSQLQuery("update igrice set igricaNaziv='"+podaci.get(0)+"', cpuPreID='"+podaci.get(1)+"', gpuPreID='"+podaci.get(2)+"',ramPreID='"+podaci.get(3)+"',imgPath = '"+imgpa+"' where igricaID='"+podaci.get(4)+"'");
                    q.executeUpdate();
                    tr.commit();
                }

                s.close();
                response.sendRedirect("ServletAdminPrikazIgrica?izmenjeno=da");
                return;
            }
            catch(HibernateException ex)
            {
